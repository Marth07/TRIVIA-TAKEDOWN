...
def main_menu():
    print("Nouveau combat prêt : Choisissez votre combattant !")
    ...
    while True:
        ...
        for text, y in options:
            is_hovered = (
                WIDTH // 2 - 150 < mouse_pos[0]
                and mouse_pos[0] < WIDTH // 2 + 150
                and y < mouse_pos[1]
                and mouse_pos[1] < y + 50
            )
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if buttons[0].collidepoint(mouse_pos):
                    print(
                        f"Lancement du Duel : {peter.name}, {steve.name}, {barry.name} et {brian.name} entre en scène !"
                    )
